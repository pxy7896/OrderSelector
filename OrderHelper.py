    ...
    while input("\nPress q to quit or press c to continue\n") != "q":
            ...
            while input("\nPress q to quit in-group operations or press c to continue\n") != 'q':
                ...
                if plate == 'a':
                    ...
                    if len(temp) == 0:
                        ...
                        for i in temp:
                            if i.date != time:
                                ...
                                result += i.filterSinglePlate(op[0], op[1])
                        # write to template excel
                        SingleTest.writeToExcel(outFile, result)
                        #resultFile.write(str(time) + "\n")
                        #SingleTest.printSourceList(li=result, file=resultFile)
                        #resultFile.close()
                    ...
                    if temp == False:
                        ...
                        SingleTest.writeToExcel(outFile, temp.source)
                        #resultFile = open(outFile, "w+")
                        #resultFile.write(" ".join([str(order), str(group)] + op)+"\n")
        ...
        # Calculations between groups
            ...
            if 'and' in op.split():
                ...
                result = SingleTest.calMulitiExpr(legalRecords, order, tmp_expr)
            else:
                info = op.split()
                ...
                try:
                    ...
                    print("\nWrong plate id. Please change groups.\n")
                    continue

            if len(outFile) < 3:
                #print("\nOrder: " + order + " " + op + "\n")
                #print(" ".join(result) + "\n")
                SingleTest.printSourceList(result)
            else:
                SingleTest.writeToExcel(outFile, result)
                #resultFile = open(outFile, "w+")
